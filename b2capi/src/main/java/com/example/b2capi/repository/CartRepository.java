...
public interface CartRepository extends JpaRepository<Cart, Long> {
    List<Cart> findAllByUserAndOrderIsNull(User user);
    ...
    @Query(value = "UPDATE `b2c`.`cart` SET `order_id` = :orderId WHERE (user_id = :userId) and (order_id is null);", nativeQuery = true)
